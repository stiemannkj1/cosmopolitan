/*
 * cosmocc-min.c - minimal Cosmopolitan/APE C-only compiler wrapper
 * ("blink-compile"). Produces fat (amd64+arm64 native) APE binaries.
 *
...
static void cmd_add(Cmd *c, const char *arg) {
  if (c->argc >= MAX_ARGS - 1) {
    fprintf(stderr, "cosmocc-min: too many arguments\n");
    exit(1);
  }
...
static int cache_is_ready(void) {
  snprintf(g_cache, sizeof g_cache, "%s/cosmocc-min/%s", cache_base_dir(), WRAPPER_VERSION);
  char ready[PATH_MAX];
  snprintf(ready, sizeof ready, "%s/cosmocc-min/%s.ready", cache_base_dir(), WRAPPER_VERSION);
...
static void populate_cache(void) {
  ...
  snprintf(parent, sizeof parent, "%s/cosmocc-min", cache_base_dir());
  mkdir_p(parent);

  if (!strncmp(g_assets, "/zip/", 5)) {
    fprintf(stderr, "cosmocc-min: extracting embedded toolchain to %s (first run)...\n", g_cache);
...
static void maybe_recommend_flags(char **extra, int nextra) {
  ...
  if (!(have_ffs && have_fds && have_gcs && have_icf)) {
    fprintf(stderr,
            "cosmocc-min: note: consider adding -ffunction-sections -fdata-sections "
...
static void usage(void) {
  fprintf(stderr,
          "usage: cosmocc-min [-c] [-o OUTPUT] [--target=amd64|arm64] [-save-temps] "
...
int main(int argc, char **argv) {
  ...
  if (nsources == 0) {
    fprintf(stderr, "cosmocc-min: no input .c files\n");
    usage();
  }
  ...
  char scratch_dir[] = "/tmp/cosmocc-min.XXXXXX";
  ...
  if (!g_save_temps) {
    ...
    fprintf(stderr, "cosmocc-min: kept intermediates in %s\n", scratch_dir);
  }
